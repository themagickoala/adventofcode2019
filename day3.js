...
for (let index = 0, tot = inputA.length; index < tot; index++) {
    const count = +inputA[index].substr(1);
    const dir = inputA[index].substr(0,1)
...
let minStepSum = undefined;

for (let index = 0, tot = inputB.length; index < tot; index++) {
    const count = +inputB[index].substr(1);
    const dir = inputB[index].substr(0,1)
    for (let i = 1; i <= count; i++) {
        switch (dir) {
            case 'R':
                x+=1;
        ...
        if (firstWire[x][y]) {
            ...
            const stepSum = firstWire[x][y] + steps;
            if (!minStepSum) {
                minStepSum = stepSum;
            } else if (stepSum < minStepSum) {
                minStepSum = stepSum;
            }
            console.log('Crossing point at ' + x + ',' + y + ' with distance ' + distance + ' and combined steps ' + stepSum);
        }
    }
}

console.log('minDistance: ' + minDistance);
console.log('minStepSum: ' + minStepSum);
