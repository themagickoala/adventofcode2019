...
function setNextPosition(tile) {
    ...
    if (map[nextY][nextX] == '.') i-=2;
    if (map[nextY][nextX] == 'O' && found) o-=2;
    if (found) {
        map[nextY][nextX] = tile == '.' ? 'O' : tile;
    } else {
        map[nextY][nextX] = map[nextY][nextX] || tile;
    }
    if (move) {
        x = nextX;
        y = nextY;
...
while(i < 100000) {
    state = intcode(state.program, state.pointer, state.relBase, dir);
    const result = state.outputs[0];
    switch(result) {
        case 0:
            setNextPosition('#');
            break;
